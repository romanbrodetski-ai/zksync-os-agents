...
The server uses three correctness-critical stages instead of a unified `Sequencer`:

- `BlockExecutor`
  - Input: `BlockCommand`
  - Output: `(BlockOutput, ReplayRecord, BlockCommandType)`
  - `OUTPUT_BUFFER_SIZE = 1`
  - Executes the block using `execute_block_in_vm`, updates in-memory mempool /
    block-context state via `on_canonical_state_change`, and appends the state diff
    to `OverlayBuffer`. **Does not persist anything.**
- `BlockCanonizer`
  - Input: `(BlockOutput, ReplayRecord, BlockCommandType)`
  - Output: `(BlockOutput, ReplayRecord, BlockCommandType)`
  - `OUTPUT_BUFFER_SIZE = 2`
  - `Replay` blocks bypass consensus and are forwarded directly downstream.
    `Produce` / `Rebuild` blocks are proposed and held behind a canonization fence
    (in `produced_queue`) until the matching canonized `ReplayRecord` arrives from
    consensus. Currently uses `NoopCanonization` (single-node loopback).
- `BlockApplier`
  - Input: `(BlockOutput, ReplayRecord, BlockCommandType)`
...
**Important ordering note:** `on_canonical_state_change` (which updates mempool state,
nonces, fee params) is called in `BlockExecutor` **before** any persistence. The mempool
therefore always reflects speculative (executed-but-not-yet-persisted) state. On crash
recovery, the mempool is rebuilt from scratch — there is no persistence ordering issue.

**Key files:**
- `lib/sequencer/src/execution/mod.rs` — exported execution stages
- `lib/sequencer/src/execution/block_executor.rs` — execution loop, overlay updates, pre-canonical mempool update
- `lib/sequencer/src/execution/execute_block_in_vm.rs` — pure VM execution function (stateless)
- `lib/sequencer/src/execution/block_canonizer.rs` — canonization fence, `BlockCanonization` trait, `NoopCanonization`
...
- `BlockContextProvider::next_block_number` — the authoritative execution cursor; `Replay`
  commands are rejected if their record block number does not match it. **Note: `Rebuild`
  commands do NOT currently check `next_block_number` — out-of-order Rebuild blocks would
  only be caught by `OverlayBuffer.add_block()`'s contiguity check.**
...
- `BlockExecutor::OUTPUT_BUFFER_SIZE = 1` — speculation ahead of canonization is bounded tightly;
  `BlockCanonizer` has its own internal `produced_queue` (capacity 2) for additional buffering
...
- `ProduceCommand` is **parameterless** (as of v0.17.0); `BlockContextProvider` owns
  `next_block_number`, `block_time`, and `max_transactions_in_block`
...
- **Block number validated** against `BlockContextProvider::next_block_number`
...
- **No `next_block_number` validation in `BlockContextProvider`** (unlike Replay);
  contiguity is only enforced later by `OverlayBuffer::add_block()`
...
The main node command source runs in three sequential phases:

1. **Replay phase**: Uses `ReadReplayExt::forward_range_with()` to forward WAL replay
   records from `starting_block` to `replay_until` as `BlockCommand::Replay`
...
3. **Produce phase**: Enters `run_loop()` and continuously emits
   `BlockCommand::Produce(ProduceCommand)` (parameterless)

The source now holds `replays_to_execute` (an `mpsc::Receiver<ReplayRecord>`), which is
fed by `BlockCanonizer` for blocks canonized by consensus. In the current main-node wiring
(single leader with `NoopCanonization`), receiving such a block during produce mode is
treated as a fatal error ("Leader node received block produced by someone else!").
...
### Speculative state (OverlayBuffer)
`BlockExecutor` accumulates executed-but-not-yet-persisted block diffs in an `OverlayBuffer`.
`BlockExecutor` may run ahead of persistence only if `OverlayBuffer` exactly mirrors the
unpersisted prefix. Two key operations:
- `sync_with_base_and_build_view_for_block(base, N)` — purges overlays already in `base`,
  builds a composite state view at block N-1 (base + overlays)
- `add_block(N, writes, preimages)` — appends block N's diff; requires N to be contiguous
  with the last entry

`OverlayBuffer` uses `Arc<BTreeMap>` to allow zero-copy sharing of the overlay snapshot
with the executing VM. The `Arc::strong_count == 1` assertion in `add_block` and
`purge_already_persisted_blocks` guards against mutation while the VM holds a view.
...
- **`replays_to_execute` loopback:** The channel from `BlockCanonizer` back to
  `MainNodeCommandSource::run_loop()` must be bounded (currently capacity 8). A full channel
  would cause `BlockCanonizer` to stall on `.await` while holding input lock — trace this if
  you see a pipeline hang during consensus integration.
...
- **Pre-canonization mempool updates:** `on_canonical_state_change()` runs in `BlockExecutor`
  before any persistence; any change that weakens abort-on-mismatch can leak speculative state
- **Rebuild block number validation:** `BlockContextProvider::prepare_command` validates
  `next_block_number` for `Replay` but not for `Rebuild`. Any PR that changes how Rebuild
  commands are dispatched should add this validation or document why it is safe to omit.
...
| Block executor | `lib/sequencer/src/execution/block_executor.rs` | Execution loop, overlay usage, pre-canonical mempool update |
| VM execution fn | `lib/sequencer/src/execution/execute_block_in_vm.rs` | Stateless VM function, SealReason |
| Block canonizer | `lib/sequencer/src/execution/block_canonizer.rs` | Canonization fence, proposal queue, BlockCanonization trait |
| Block applier | `lib/sequencer/src/execution/block_applier.rs` | Persistence ordering, override policy |
| BlockContextProvider | `lib/sequencer/src/execution/block_context_provider.rs` | Tx selection, block context, next_block_number |
| Command source | `node/bin/src/command_source.rs` | WAL replay forwarding, rebuild emission, produce loop, replays_to_execute channel |
| Transaction pool | `lib/mempool/src/pool.rs` | Subpool priority ordering |
| Replay storage | `lib/storage_api/src/replay.rs` | ReadReplay/WriteReplay traits, forward_range_with |
...
| Overlay buffer | `lib/storage_api/src/overlay_buffer.rs` | Unpersisted state diffs, Arc-based sharing, contiguity enforcement |
